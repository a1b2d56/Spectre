...
interface SshAgentPacketChannel {
    fun readPacket(): ByteArray?
    fun writePacket(packet: ByteArray)
}

fun hkdfSha256(seed: ByteArray, salt: ByteArray, info: ByteArray, length: Int): ByteArray {
    val mac = Mac.getInstance("HmacSHA256")
    val saltSpec = if (salt.isEmpty()) {
    ...
    val blocks = ceil(length.toDouble() / 32).toInt()
    var offset = 0
    for (i in 1..blocks) {
        ...
        System.arraycopy(currentBlock, 0, result, offset, copyLen)
        offset += copyLen
    }
    return result
}
...
fun InputStream.readNBytesCompat(len: Int): ByteArray {
    ...
    var read = 0
    while (read < len) {
        val count = this.read(result, read, len - read)
        if (count == -1) break
        read += count
    }
    if (read < len) {
        throw EOFException("Reached EOF before reading expected $len bytes")
    }
    return result
}
